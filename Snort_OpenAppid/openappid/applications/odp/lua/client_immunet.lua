...
--]]
--[[
detection_name: FireAMP
version: 9
description: Cloud-based real time antivirus protection.
--]]
...
gPatterns = {
	udp_version = {"\003", 0, gSfAppIdImmunet},
    tcp_version = {"\003", 4, gSfAppIdImmunet},
}


gFastPatterns = {
    {DC.ipproto.udp, gPatterns.udp_version},
    {DC.ipproto.tcp, gPatterns.tcp_version},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdImmunet);
...
function client_init( detectorInstance, configOptions)
    ...
    gDetector:addHttpPattern(2, 5, 0, 110, 23, 0, 0, 'ImmunetProtect', 934, 1);
    gDetector:addHttpPattern(2, 5, 0, 110, 23, 0, 0, 'ClamAV', 934, 1);
    gDetector:addAppUrl(0, 110, 23, 0, 0, "update.immunet.com", "/", "http:", "", 934);
    gDetector:addAppUrl(0, 110, 23, 0, 0, "dl.local.win.clamav.net", "/daily", "http:", "", 934);
...
function client_validate()
	...
	if (size >= 3 and dir == 0) then
        if  (dstPort == 32137 or (dstPort == 443 and isFlowTcp(flowKey))) then
