...
--]]
--[[
detection_name: Jabber
description: Opensource XMPP client.
--]]
...
gPatterns = {
    xml = {"<?xml version=", 0, gSfAppIdJabber},
    str = {"<stream:stream", 0, gSfAppIdJabber},
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.xml},
    {DC.ipproto.tcp, gPatterns.str},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdJabber);
...
function client_validate()
    ...
    if ((dir == 0) and
        (size >= 35) and
        (gDetector:getPcreGroups("stream:stream")))
    then
