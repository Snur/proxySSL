...
function client_validate()
	...
	if ((dir == 0) and
       (size >= 26) and
	   (dstPort == 3389) and
       (gDetector:memcmp(gPatterns.tpkt[1], #gPatterns.tpkt[1], gPatterns.tpkt[2]) == 0))
    then
        matched, byte_four_raw = gDetector:getPcreGroups("(.)",4)
        byte_four = DC.binaryStringToNumber(byte_four_raw, 1)
        if (byte_four == size-5) then
            if (gDetector:getPcreGroups("Cookie: mstshash=")) then
