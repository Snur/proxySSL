...
gPatterns = {
	connect = {'\003\000\090\006\006\005', 0, gSfAppIdAres},
	udpBootstrap = {'\233\085',0, gSfAppIdAres},
	udpChat = {'\233\096',0, gSfAppIdAres},
}


gFastPatterns = {
	{DC.ipproto.tcp, gPatterns.connect},
	{DC.ipproto.udp, gPatterns.udpBootstrap},
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdAres);
...
function client_validate()
	...
	--actual downloading is encrypted, so only checking for the initial connection message	
	if ((size == 6) and
		(gDetector:memcmp(gPatterns.connect[1], #gPatterns.connect[1], gPatterns.connect[2]) == 0))
	then
		DC.printf("connect\n")
		return clientSuccess(context)
	end

	--checks for the potential UDP messages
	if ((size == 27) and
		(gDetector:memcmp(gPatterns.udpBootstrap[1], #gPatterns.udpBootstrap[1], gPatterns.udpBootstrap[2]) == 0))
	then
		DC.printf("Bootstrap\n")
		return clientSuccess(context)
	end
	...
	if ((size == 34) and
		(gDetector:memcmp(gPatterns.udpChat[1], #gPatterns.udpChat[1], gPatterns.udpChat[2]) == 0))
	then
		DC.printf("chat\n")
		return clientSuccess(context)
	end
