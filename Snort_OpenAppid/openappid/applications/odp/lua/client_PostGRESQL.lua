...
gSfAppIdPostgresSql = 791

gPatterns = {
	userPattern = {'user\000', 8, gSfAppIdPostgresSql},	
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.userPattern},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdPostgresSql);
...
function client_validate()
	...
	if ((size >= 22) and
		(gDetector:memcmp(gPatterns.userPattern[1], #gPatterns.userPattern[1], gPatterns.userPattern[2])) and
	    (gDetector:getPcreGroups("database")))
	then
		matched, rawLength = gDetector:getPcreGroups("(....)", 0)
		length = DC.binaryStringToNumber(rawLength, 4)
		if (length == size) then
