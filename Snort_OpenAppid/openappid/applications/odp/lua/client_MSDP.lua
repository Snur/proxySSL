...
gPatterns = {
	MSDPHeader = {'\004\000\003', 0, gSfAppIdMsdp},
}


gFastPatterns = {
	{DC.ipproto.tcp, gPatterns.MSDPHeader},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdMsdp);
...
function client_validate()
	...
	if (dstPort == 639) then
		matched, rawSize = gDetector:getPcreGroups('\004(..)', 0)
		if (matched) then
			proposedSize = DC.binaryStringToNumber(rawSize, 2)
			if (proposedSize == size and size == 3) then
