...
gPatterns = {
	pattern1 = {"\000\000\000\000", 0, gSfAppIdPcAnywhere},
	pattern2 = { "NQ", 0, gSfAppIdPcAnywhere},
	pattern3 = { "ST", 0, gSfAppIdPcAnywhere},
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdPcAnywhere);
...
function client_validate()
	...
	if ((size == 4) and
		...
		(not gFBsaw_greeting))
	then
		gFBsaw_greeting = true
		return clientInProcess(context)
	end

	if ((gFBsaw_greeting) and
		(dstPort == 5631) and
		(gDetector:memcmp(gPatterns.clientHeader[1], #gPatterns.clientHeader[1], gPatterns.clientHeader[2]) == 0))
	then
		gFBsaw_greeting = false
	...
	if ((size == 2) and
		(dstPort == 5632) and
        ((gDetector:memcmp(gPatterns.pattern2[1], #gPatterns.pattern2[1], gPatterns.pattern2[2]) == 0) or
