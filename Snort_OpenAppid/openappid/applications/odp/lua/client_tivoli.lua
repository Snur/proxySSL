...
gPatterns = {
	client_req = {'\029\165', 2, gSfAppIdTivoli},
}


gFastPatterns = {
	{DC.ipproto.tcp, gPatterns.client_req},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdTivoli)
...
function client_validate()
    ...
    DC.printf ('Tivoli client packetCount %d dir %d, size %d\n', context.packetCount, dir, size)

	if (size == 0) then
    	return clientInProcess(context)
	end

	if (dstPort == 1500 or dstPort == 1580 or dstPort == 1581 or dstPort == 1900) then
		matched, len_raw = gDetector:getPcreGroups('(..)', 0)
		if (matched) then
			len = DC.binaryStringToNumber(len_raw, 2)
            DC.printf('tivoli client len is %d size is %d\n', len, size)
			if (len == size) then
