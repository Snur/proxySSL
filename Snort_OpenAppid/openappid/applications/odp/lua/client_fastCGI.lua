...
--]]
--[[
detection_name: FastCGI
version: 3
description: Distributed extension of CGI.
--]]
...
gSfAppIdFastCGI = 1440

gPatterns = {
	first = {'\001\000\001', 1, gSfAppIdFastCGI}, 
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, context.version, gSfAppId);
...
function client_validate()
    ...
    DC.printf ('FastCGI packetCount %d dir %d, size %d\n', context.packetCount, dir, size)
	...
	if (size >= 8) then
        matched, ver, len = gDetector:getPcreGroups("(.)...(..)", 0)
        if (matched) then
            DC.printf("FastCGI matched it\n")
            context.version = DC.binaryStringToNumber(ver, 1)
            actual_len = DC.binaryStringToNumber(len, 2)
            if (size - 8 == actual_len) then
