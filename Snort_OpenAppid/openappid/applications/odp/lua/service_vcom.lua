...
gPatterns = {       
    ...
    --------------------------------------------------------------------
    pattern1          = {'VCOM_SRV',                57, gSfAppIdVcom},
}
...
function serviceSuccess(context)
    ...
    if ((not flowFlag) or (flowFlag == 0)) then
        gDetector:addService(gServiceId, context.vendor, context.version, gSfAppIdVcom)
    end
...
function DetectorValidator()
    ...
    if (dir == DC.flowDirection.fromResponder) then
        if (size >= 65 and (gDetector:memcmp(gPatterns.pattern1[1], #gPatterns.pattern1[1], 
            gPatterns.pattern1[2]) == 0)) then 
            matched, version = gDetector:getPcreGroups("VCOM_SRV_VERSION0*([0-9]*\\.[0-9]*\\.[0-9])", 101)
            if matched then
