...
gSfAppIdCvs = 597
gPatterns = {
    auth_req = { "BEGIN AUTH REQUEST", 0, gSfAppIdCvs},
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.auth_req},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdCvs);
...
function client_validate()
    ...
    if ((dir == 0) and 
        (size >= 18) and
        (gDetector:memcmp(gPatterns.auth_req[1], #gPatterns.auth_req[1], gPatterns.auth_req[2]) == 0) and
        (gDetector:getPcreGroups("END AUTH REQUEST"))) 
    then
