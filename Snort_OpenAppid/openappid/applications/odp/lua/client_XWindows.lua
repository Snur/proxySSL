...
gPatterns = {
	initHeader1 = {'\000\011\000\000', 2, gSfAppIdXWindows},
	initHeader2 = {'\011\000\000\000', 2, gSfAppIdXWindows},
}

gFastPatterns = {
	{DC.ipproto.tcp, gPatterns.initHeader1},
	{DC.ipproto.tcp, gPatterns.initHeader2},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdXWindows);
...
function client_validate()
	...
	if ((sawClientInit) and
		(gDetector:getPcreGroups("BIG-REQUESTS")))
	then
		sawClientInit = false
		return clientSuccess(context)
	end

	if ((size >= #gPatterns.initHeader1[1] + 6) and
		(not sawClientInit))
	then	
		matched, group1, group2 = gDetector:getPcreGroups('(.).(....)', 0) 
		if ((matched) and 
			(group1 == 'B' or group1 == 'l') and 
			(group2 == '\000\011\000\000' or group2 == '\011\000\000\000'))
		then
			if (dstPort >= 6000 and dstPort <= 6002) then
				return clientSuccess(context)
			else --if port doesn't match, check next packet
				sawClientInit = true	
				return clientInProcess(context)
			end
		end
