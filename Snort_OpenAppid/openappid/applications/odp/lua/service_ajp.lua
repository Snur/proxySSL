...
detection_name: AJP
version: 4
description: Communications between webservers and servlet repositories.
--]]
...
function serviceSuccess(context)
    ...
    if ((not flowFlag) or (flowFlag == 0)) then
        gDetector:addService(gServiceId, "", "", gSfAppIdAjp)
    end
...
function DetectorValidator()
    ...
    if (dir == DC.flowDirection.fromResponder) then
        if (size >= 5 and (gDetector:memcmp(gPatterns.pattern1[1], #gPatterns.pattern1[1], 
