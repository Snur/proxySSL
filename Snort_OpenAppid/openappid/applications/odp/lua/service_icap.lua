...
detection_name: ICAP
version: 5
description: Proxy service for specific content.
--]]
...
function serviceSuccess(context)
    ...
    if ((not flowFlag) or (flowFlag == 0)) then
        gDetector:addService(gServiceId, "", "", gSfAppIdIcap)
    end
...
function DetectorValidator()
    ...
    if (dir == 1 and size >= 5 and 
       (gDetector:memcmp(gPatterns.pattern[1], #gPatterns.pattern[1], gPatterns.pattern[2]) == 0)) then 
        DC.printf ('%s:DetectorValidator(): matched pattern\n', gServiceName)
        return serviceSuccess(context)
    end

    if (context.packetCount > 25) then
