...
--]]
--[[
detection_name: 100Bao
version: 1
description: A Chinese P2P file sharing program.
--]]
...
gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.cmd, 0, gSfAppId100Bao},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppId100Bao);
...
function client_init( detectorInstance, configOptions)
    ...
    for i,v in ipairs(gFastPatterns) do
        if ( gDetector:client_registerPattern(v[1], v[2][1], #v[2][1], v[3], v[4]) ~= 0) then
...
function client_validate()
    ...
    if (dir == 0) then
        if ((size >= 77) and
            (gDetector:getPcreGroups("Content-Length")) and
            (gDetector:getPcreGroups("Connection")) and
            (gDetector:getPcreGroups("Request-ID:")) and
            (gDetector:getPcreGroups("System:")))
        then
