...
detection_name: DB2
version: 1
description: IBM's relational database.
--]]
...
gPatterns = {
    string1 = { "DB2DAS", 4, gSfAppIdDB2},
    string2 = { "9z", 22, gSfAppIdDB2},
}

gFastPatterns = {
    {DC.ipproto.tcp, gPatterns.string1},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdDB2);
...
function client_validate()
    ...
    if ((dir == 0) and 
        (size == 41) and
        (gDetector:memcmp(gPatterns.string2[1], #gPatterns.string2[1], gPatterns.string2[2]) == 0))
    then
