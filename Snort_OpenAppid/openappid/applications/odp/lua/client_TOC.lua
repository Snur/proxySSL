...
detection_name: TOC
version: 1
description: Instant messaging protocol.
--]]
...
gPatterns = {
	clientSignOn = {"FLAPON\013\010\013\010", 0, gSfAppIdToc}
}

gFastPatterns = {
	{DC.ipproto.tcp, gPatterns.clientSignOn},
}
...
function clientSuccess(context)
    ...
    gDetector:client_addApp(appServiceId, appTypeId, appProductId, "", gSfAppIdToc);
...
function client_validate()
	...
	--Looks at the first 3 client packets. The initial sign on message, the flap sign on message, and then the TOC signon/login message
	if ((size == 10) and
		(not sawSignOn and not sawFlapSignOn) and
		(gDetector:memcmp(gPatterns.clientSignOn[1], #gPatterns.clientSignOn[1], gPatterns.clientSignOn[2]) == 0))
	then
		sawSignOn = true
		return clientInProcess(context)
	end
	...
	if ((sawSignOn and not sawFlapSignOn) and
		(gDetector:memcmp('\042\001', 2, 0) == 0))
	then
		sawFlapSignOn = true
		return clientInProcess(context)
	end
	...
	if ((sawFlapSignOn) and
		(size >= 10) and
		(gDetector:getPcreGroups("toc2_login") or gDetector:getPcreGroups("toc_signon")))
	then
		sawSignOn = false
		sawFlapSignOn = false
