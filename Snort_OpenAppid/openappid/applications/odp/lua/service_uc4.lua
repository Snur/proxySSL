...
function serviceSuccess(context)
    ...
    if ((not flowFlag) or (flowFlag == 0)) then
        gDetector:addService(gServiceId, "UC4", "", gSfAppIdUc4)
    end
...
function registerPortsPatterns()
    ...
    DC.printf('registering patterns\n')
...
function DetectorValidator()
    ...
    if (dir == DC.flowDirection.fromResponder) then
        if (size >= 17 and (gDetector:memcmp(gPatterns.pattern1[1], #gPatterns.pattern1[1], 
            gPatterns.pattern1[2]) == 0)) then 
            DC.printf ('checking regexp\n')
            matched = gDetector:getPcreGroups("UC4 [A-Z]*-Executor", 4)
            if matched then
