...
--]]
--[[
detection_name: Zabbix Trap
version: 4
description: Zabbix network management system trapper.
--]]
...
function DetectorValidator()
    ...
    context.detectorFlow:setFlowFlag(DC.flowFlags.udpReversed)
    if (size >= 5 and dir == DC.flowDirection.fromInitiator) then
        if (gDetector:memcmp(headBanner, #headBanner, 0) == 0) then
            return serviceSuccess(context)
        end
    end

    DC.printf ('%s:DetectorValidator(): error should not have received packet froms server\n', gServiceName);
